...
- Formal default and `--quick-generate` release export compute the exact SVG source fingerprint and normally refuse a missing, unreadable, unsupported, non-final, blocking, stale, or unverifiable final quality report before PPTX creation. `--allow-quality-warnings` permits only a current, final, hash-matched report whose status is blocking to continue as a user-inspectable export; missing, unreadable, unsupported, non-final, stale, or unverifiable reports remain fatal. A project without `validation/svg_quality_report.json` exits nonzero with the `not-provided` gate status; run the final checker against its current `svg_output/` first. An explicit non-`output` `--source` remains a diagnostic override and bypasses this release gate; postflight still records any verifiable report linkage.
