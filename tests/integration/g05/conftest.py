...
def database_url() -> str:
    value = os.environ.get("G05_TEST_DATABASE_URL")
    if not value:
        pytest.fail("G05_TEST_DATABASE_URL is required; use pnpm verify:integration:g05")
    return value
...
def clean_g05_state(session_factory: sessionmaker[Session]) -> None:
    tables = (
        "outline_slides",
        "outline_approvals",
        "outline_revisions",
