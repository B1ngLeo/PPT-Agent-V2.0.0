...
def _revision(
    ...
    *,
    ...
    response = client.post(
        ...
        json=_mutation({"operations": operations}, presentation["currentRevisionId"]),
    )
...
def test_operation_set_is_immutable_optimistic_and_retains_stable_slide_ids(
    ...
    foreign = client.get(
        f"/v1/presentations/{presentation['presentationId']}", headers=BOB
    )
...
def test_regeneration_keeps_old_ready_version_until_qa_then_switches_atomically(
    ...
    queued = client.post(
        ...
        json=_mutation(
            {"instruction": "把行动写得更明确"}, presentation["currentRevisionId"]
        ),
    )
    ...
    status = client.get(
        f"/v1/operations/{operation['regenerationJobId']}", headers=ALICE
    ).json()["data"]
...
def test_export_binds_exact_revision_while_concurrent_edit_advances_current(
    ...
    queued = client.post(
        ...
        json=_mutation(
            {"presentationRevisionId": export_revision_id}, export_revision_id
        ),
    )
    assert queued.status_code == 202, queued.text
    ...
    completed = client.get(f"/v1/exports/{export['exportId']}", headers=ALICE).json()[
        "data"
    ]
    ...
    with session_factory() as session:
        ...
        assert manifest["artifact"]["artifactType"] == "export_pptx"
        assert pptx_artifact.artifact_type == "export_pptx"
...
def test_partial_requires_decision_and_delete_revokes_all_routes_and_objects(
    ...
    assert (
        client.get(f"/v1/jobs/{created['job']['jobId']}", headers=ALICE).status_code == 404
    )
    assert (
        client.get(
            f"/v1/jobs/{created['job']['jobId']}/events", headers=ALICE
        ).status_code
        == 404
    )
    assert (
